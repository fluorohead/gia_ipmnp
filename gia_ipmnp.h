...
public:
    ...
    u32i get_oui() { fix(); return as_48bits >> 24; };
    bool is_ucast() { return (as_u8i[macmnp::oct1] & 0b00000001) ? false : true; };
    bool is_mcast() { return !is_ucast(); };
    bool is_bcast() { return as_48bits == 0xFFFFFFFFFFFF; };
    bool is_uaa() { return (as_u8i[macmnp::oct1] & 0b00000010) ? false : true; };
    bool is_laa() { return !is_uaa(); };
    MAC_Addr operator&(u64i bitmask) { return MAC_Addr{as_48bits & bitmask}; };
    MAC_Addr operator&(const MAC_Addr &bitmask) { return MAC_Addr{as_48bits & bitmask.as_48bits}; };
    void operator&=(u64i bitmask) { as_48bits &= bitmask; };
    void operator&=(MAC_Addr &bitmask) { as_48bits &= bitmask.as_48bits; };
    bool operator>(u64i _48bits) { return as_48bits > _48bits; };
    bool operator>(MAC_Addr mac) { return as_48bits > mac.as_48bits; };
    bool operator<(u64i _48bits) { return as_48bits < _48bits; };
    bool operator<(MAC_Addr mac) { return as_48bits < mac.as_48bits; };
    bool operator>=(u64i _48bits) { return as_48bits >= _48bits; };
    bool operator>=(MAC_Addr mac) { return as_48bits >= mac.as_48bits; };
...
public:
    ...
    bool is_loopback() { return (as_u32i & 0xFF000000) == 0x7F000000; }; // 127/8 - RFC 1122
    bool is_link_local() { return (as_u32i & 0xFFFF0000) == 0xA9FE0000;  }; // 169.254/16 - RFC 3927
    bool is_lim_bcast() { return as_u32i == UINT32_MAX; }; // 255.255.255.255/32 - RFC 6890
    bool is_mcast() { return (as_u32i & 0xF0000000) == 0xE0000000; }; // 224/4
    bool is_ssm() { return (as_u32i & 0xFF000000) == 0xE80000; }; // 232/8
    bool is_glop() { return (as_u32i & 0xFF000000) == 0xE90000; }; // 233/8 - RFC 3180
    bool is_ubm() { return (as_u32i & 0xFF000000) == 0xEA0000; } // 234/8 - RFC 6034
    bool is_ucast() { return !is_mcast(); };
    bool is_as112() { return (as_u32i & 0xFFFFFF00) == 0xC01FC400; }; // 192.31.196/24 - RFC 7535
    bool is_glob_ucast();
    bool is_shared() { return (as_u32i & 0xFFC00000) == 0x64400000; }; // 100.64/10 - RFC 6598
    bool is_reserved() { return (as_u32i & 0xF0000000) == 0xF0000000; }; // 240/4 - RFC 6890
    bool is_docum(); // 192.0.2/24, 198.51.100/24, 203.0.113/24 - RFC 5737
    bool is_benchm() { return (as_u32i & 0xFFFE0000) == 0xC6120000; }; // 198.18/15 - RFC 2544
    bool is_ietf() { return (as_u32i & 0xFFFFFF00) == 0xC0000000; }; // 192/24 - RFC 6890
    bool is_dslite() { return (as_u32i & 0xFFFFFFF8) == 0xC0000000; }; // 192/29 - RFC 6333, RFC 7335
    bool is_amt() { return (as_u32i & 0xFFFFFF00) == 0xC034C100; }; // 92.52.193/24 - RFC 7450
    bool is_dirdeleg() { return (as_u32i & 0xFFFFFF00) == 0xC0AF3000; }; // 192.175.48/24 - RFC 7534
    IPv4_Addr operator+(u32i sum) { return IPv4_Addr{as_u32i + sum}; };
    ...
    IPv4_Addr operator<<(u32i shift) { return IPv4_Addr{as_u32i << shift}; };
    IPv4_Addr operator>>(u32i shift) { return IPv4_Addr{as_u32i >> shift}; };
    void operator<<=(u32i shift) { as_u32i <<= shift; };
    void operator>>=(u32i shift) { as_u32i >>= shift; };
    IPv4_Addr operator&(u32i bitmask) { return IPv4_Addr{as_u32i & bitmask}; };
    IPv4_Addr operator&(const IPv4_Addr &bitmask) { return IPv4_Addr{as_u32i & bitmask.as_u32i}; };
    void operator&=(u32i bitmask) { as_u32i &= bitmask; };
    void operator&=(const IPv4_Addr &bitmask) { as_u32i &= bitmask.as_u32i; };
    bool operator>(u32i ip) { return as_u32i > ip; };
    bool operator>(IPv4_Addr ip) { return as_u32i > ip.as_u32i; };
    bool operator<(u32i ip) { return as_u32i < ip; };
    bool operator<(IPv4_Addr ip) { return as_u32i < ip.as_u32i; };
    bool operator>=(u32i ip) { return as_u32i >= ip; };
    bool operator>=(IPv4_Addr ip) { return as_u32i >= ip.as_u32i; };
...
class IPv6_Addr {
    bool getzg(u32i *beg, u32i *end); // finds longest group of zero-hextets
    bool show_ipv4 {true};
public:
    union {
    ...
    bool is_loopback() { return (as_u64i[0] | as_u64i[1]) == 1; }; // ::/128 - RFC 4291
    bool is_glob_ucast() { return (as_u16i[v6mnp::xtt1] & 0xFFE0) == 0x2000; }; // 2000::/3 - RFC 3513
    bool is_mcast() {return (as_u16i[v6mnp::xtt1] & 0xFF00) == 0xFF00; }; // ff00::/8 - RFC 3513
    bool is_uniq_local() { return (as_u16i[v6mnp::xtt1] & 0xFE00) == 0xFC00; }; // fc00::/7 - RFC 4193
    bool is_link_local() { return (as_u16i[v6mnp::xtt1] & 0xFFC0) == 0xFE80; }; // fe80::/10 - RFC 4862
    bool is_mapped_ipv4() { return as_u16i[v6mnp::xtt6] == 0xFFFF; }; // ::ffff:0:0/96 - RFC 4291
    bool is_wknown_pfx() { return (as_u64i[1] == 0x0064FF9B00000000) && (as_u32i[1] == 0x00000000); } // 64:ff9b::/96 - RFC 6052
    bool is_lu_trans() { return (as_u32i[3] == 0x0064FF9B) && (as_u16i[v6mnp::xtt3] == 0x0001); }; // 64:ff9b:1::/48  - RFC 8215
    bool is_ietf() { return (as_u32i[3] & 0xFFFFFE) == 0x20010000; }; // 2001:0::/23 - RFC2928
    bool is_teredo() { return as_u32i[3] == 0x20010000; }; // 2001:0::/32 - RFC4380
    bool is_benchm() { return (as_u32i[3] == 0x20010002) && (as_u16i[v6mnp::xtt3] == 0x0000); }; // 2001:2::/48  - RFC 5180
    bool is_amt() { return as_u32i[3] == 0x20010003; }; // 2001:3::/32 - RFC 7450
    bool is_as112() { return (as_u32i[3] == 0x20010004) && (as_u16i[v6mnp::xtt3] == 0x0112); }; // 2001:4:112::/48 - RFC 7535
    bool is_orchv2() { return (as_u32i[3] & 0xFFFFFFF0) == 0x20010020; }; // 2001:20::/28 - RFC 7343
    bool is_docum() { return as_u32i[3] == 0x20010DB8; } // 2001:db8::/32 - RFC 3849
    bool is_6to4() { return as_u16i[v6mnp::xtt1] == 0x2002; }; // 2002::/16 - RFC 3056
    ...
    IPv6_Addr operator+(IPv6_Addr sum);
    IPv6_Addr operator+(u64i sum);
    IPv6_Addr operator-(IPv6_Addr sub);
    IPv6_Addr operator-(u64i sub);
    ...
    void operator+=(IPv6_Addr sum);
    void operator+=(u64i sum);
    void operator-=(IPv6_Addr sub);
    void operator-=(u64i sub);
    IPv6_Addr operator&(const IPv6_Addr &bitmask) { return IPv6_Addr{as_u64i[1] & bitmask.as_u64i[1], as_u64i[0] & bitmask.as_u64i[0]}; };
    void operator&=(const IPv6_Addr &bitmask) { as_u64i[1] &= bitmask.as_u64i[1]; as_u64i[0] &= bitmask.as_u64i[0]; };
    bool operator==(IPv6_Addr ip) { return (as_u64i[1] == ip.as_u64i[1]) && (as_u64i[0] == ip.as_u64i[0]); };
    bool operator!=(IPv6_Addr ip) { return (as_u64i[1] != ip.as_u64i[1]) || (as_u64i[0] != ip.as_u64i[0]); };
    bool operator>(IPv6_Addr ip);
    bool operator<(IPv6_Addr ip);
    bool operator>=(IPv6_Addr ip);
    bool operator<=(IPv6_Addr ip);
    IPv6_Addr operator<<(u32i shift);
