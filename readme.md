...
Класс IPv6_Addr тоже поддерживает **безопасный доступ (с контролем выхода из диапазона)** к своему целочисленному значению через **индексацию квадратными скобками**, применёнными с имени объекта. При выходе за пределы массива возвращается 0 либо случайное число.
