...
Имеется так же прямой доступ на запись и на чтение к отдельным октетам через массив **`.as_u8i`**. Такой способ **является опасным!** Неверная индексация при операции присваивания приведёт к выходу за пределы массива, что может повлечь порчу данных, крэшу процесса и т.д. Скорее всего, в окончательной версии этот способ будет удалён, поэтому уже сейчас не рекомендуется его использовать.
...
Более **предпочтителен** метод индексации через **квадратные скобки**, применённые к самому имени объекта. При выходе за пределы массива, будет возвращён 0 либо случайное число, поэтому такой **метод является безопасным**.
