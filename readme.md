...
Для возврата целочисленного представления рекомендуется использовать более компактный вызов имени объекта с круглыми скобками, подобно вызову функции :
...
Имеется так же прямой доступ на запись и чтение к отдельным октетам через массив **`IPv4_Addr::as_u8i`**, но такой способ **является опасным!** Неверная индексация при операции присваивания приведёт к выходу за пределы массива, что может повлечь порчу данных, крэшу процесса и т.д. Скорее всего, в окончательной версии этот способ будет удалён, поэтому его не рекомендуется использовать уже сейчас.
...
Здесь есть все те же опасности прямого доступа, как и для типов IPv4_Addr, IPv6_Addr. Чтобы исключить путаницу, были созданы константы **`macmnp::oct1 - oct6`**. Рекомендуется всегда использовать только их.

Например для адреса 38:68:93:80:07:E6 обращение **`MAC_Addr.as_u8i[macmnp::oct1]`** вернёт 0x38, обращение **`MAC_Addr.as_u8i[macmnp::oct2]`** вернёт 0x68 и т.д.
...
Некоторые способы прямого доступа через **MAC_Addr::as_u8i** позволяют выставить ненулевые значения 2 старших байтов. Метод **`MAC_Addr::fix()`** обнуляет данные значения, т.к. они не должны использоваться.
