...
Это тоже классы, но наследование от них не имеет большого смысла, т.к. все их методы статические и используются в свою очередь в IPv4_Addr, IPv6_Addr, и MAC_Addr.
...
Здесь есть все те же **опасности прямого доступа**, как и для типов IPv4_Addr, IPv6_Addr. Чтобы исключить путаницу, были созданы константы **`macmnp::oct1 - oct6`**. Рекомендуется всегда использовать только их, но индексировать не массив **`MAC_Addr::as_u8i`**, а сам объект :
