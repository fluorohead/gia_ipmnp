...
Прямой доступ на запись и чтение к отдельным октетам через массив **`IPv4_Addr::as_u8i`** возможен, но **является опасным!** Неверная индексация при операции присваивания приведёт к выходу за пределы массива, что может повлечь порчу данных, крэшу процесса и т.д. Скорее всего, в окончательной версии этот способ будет удалён, поэтому его не рекомендуется использовать уже сейчас.
