...
Назначение псевдонимов чисто номинальное : наглядная дифференциация адресов и масок в коде Вашего проекта. Псевдонимы и их классы полностью взаимозаменяемы.
...
При инициализации из строки всегда происходит валидация синтаксиса, и в случае неудачи объект инициализируется адресом **0.0.0.0**.
...
--
Внутреннее представление адреса целочисленно, поэтому для класса сохранены некоторые арифметические, битовые и операции сравнения : + - ++ -- += -= << >> <<= >>= & &= > < >= <= == != ~
...
На платформе **Intel/AMD x86-x64** очерёдность расположения октетов в памяти реверсивна относительно передачи по линиям связи, поэтому доступ рекомендуется выполнять, используя константы **v4mnp::oct1, v4mnp::oct2, v4mnp::oct3, v4mnp::oct4**.
...
--
**Возвращает байтовый массив из 4 элементов (32 бита) в порядке, аналогичном порядку среды передачи данных (старшие октеты передаются первыми)** :
...
-
**Возвращает байтовый массив из 16 элементов (128 бит) в порядке, аналогичном порядку среды передачи данных (старшие октеты передаются первыми)** :
...
Для правильного отображения маски необходимо вызвать **unsetflag_show_ipv4()**. Метод **setflag_show_ipv4()** возвращает поведение по умолчанию, когда флаг в состоянии **true**. Для адресов не требуется вызывать **setflag_show_ipv4()**.
...
**Подсчёт длины маски** :
    ...
    static u32i mask_len(const IPv6_Mask &mask)
**Не производит валидацию на непрерывность**, подсчитывает только крайние правые нули до первой двоичной единицы .
...
**Старшие 2 байта числа u64i(as_48bits) (и соответствующие октеты as_u8i[7], as_u8i[6]) не используются. Если есть вероятность, что прямой доступ привёл к выставлению ненулевых значений, нужно воспользоваться методом fix().**

Дополнительные методы класса *MAC_Addr* 
-
**Возвращает  байтовый массив из 6 элементов в порядке следования байтов, как в среде передачи данных (старшие октеты передаются первыми)**:
 ...
 **Обнуление старших 2 байтов представления u64i(as_48bits)** :
     ...
     void fix()
Некоторые способы прямого доступа через as_u8i позволяют выставить ненулевые значения 2 старших байтов. Метод fix() обнуляет данные значения, т.к. они не должны использоваться.
