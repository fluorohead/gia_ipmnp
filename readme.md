...
При инициализации массивом, порядок следования значений - справа-налево, как и в символьном представлении :
...
--
Внутреннее представление адреса целочисленно, поэтому сохранены целочисленные операторы :
 \+ - ++ -- += -= << >> <<= >>= & &= | |= > < >= <= == != ~ =
...
Поддерживается прямой доступ к целочисленному значению через переменную-член **`.as_u32i`** :
...
Имеется так же прямой доступ к отдельным октетам через массив **`.as_u8i`**, как на запись, так и на чтение. Такой способ **является небезопасным!**. Возможен выход за пределы массива, что может привести к порче данных, крэшу процесса и прочим видам UB. Скорее всего, в окончательной версии такой способ будет удалён, поэтому уже сейчас не рекомендуется его применять.

На платформе **Intel/AMD x86-x64** очерёдность расположения октетов в памяти реверсивна относительно передачи по линиям связи, поэтому доступ правильнее выполнять, используя константы **`v4mnp::oct1, v4mnp::oct2, v4mnp::oct3, v4mnp::oct4`**.

Более **предпочтителен** метод индексации через **квадратные скобки**, применённый к имени объекта IPv4_Addr. При выходе за пределы массива, будет возвращён 0 либо случайное число.
