...
Здесь есть все те же **опасности**, как и для типов IPv4_Addr, IPv6_Addr. Поэтому, чтобы исключить путаницу и критические ошибки, были созданы константы **`macmnp::oct1 - oct6`**. Рекомендуется всегда использовать только их и индексировать не массив **`MAC_Addr::as_u8i`**, а сам объект :
